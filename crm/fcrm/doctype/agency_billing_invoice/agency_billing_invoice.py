...
class AgencyBillingInvoice(Document):
	...
	if TYPE_CHECKING:
		...
		stripe_hosted_invoice_url: DF.SmallText | None
		stripe_invoice_id: DF.Data | None
		stripe_payment_intent_id: DF.Data | None
		stripe_status: DF.Data | None
		subtotal: DF.Currency
		total: DF.Currency
		trial_waived: DF.Check
	# end: auto-generated types

	def validate(self):
		self.subtotal = flt(sum(flt(row.amount or 0) for row in self.items))
		self.total = self.subtotal
		# Keep Trial Waived flag consistent even if invoice is edited later.
		is_waived = any((row.description or "").startswith("(Trial Waived)") for row in (self.items or []))
		self.set("trial_waived", 1 if is_waived else 0)
